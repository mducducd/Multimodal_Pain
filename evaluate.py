...
from torchmetrics import Accuracy, AUROC, F1Score, ConfusionMatrix, CohenKappa, Recall, Recall, AveragePrecision
...
def train_celebvhq(args, config):
    ...
    ckpt_filename = config["model_name"] + "-{epoch}-{val_acc:.3f}-{val_auc:.3f}"
...
def evaluate_celebvhq(args, ckpt, dm):
    ...
    for i, (_, y) in enumerate(tqdm(dm.val_dataloader())):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default='../AI4Pain_Dataset_full', help="Path to CelebV-HQ dataset.")
    ...
    # )
    # dm.setup()
    # evaluate(args,'/home/hdd1/duke/AI4pain/MARLIN/ckpt/celebvhq_marlin_large_convtrans_fz_fc_unfz/celebvhq_marlin_large_convtrans_fz_fc_unfz-epoch=363-val_acc=0.889-val_auc=0.427.ckpt',dm)
