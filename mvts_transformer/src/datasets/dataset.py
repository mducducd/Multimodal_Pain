    ...
    def __getitem__(self, ind):
        ...
        for i in range(X.shape[1]):
            ...
            if col_min != col_max:  # Avoid division by zero
                X[:, i] = 2 * (X[:, i] - col_min) / (col_max - col_min) - 1
            else:
                X[:, i] = 0  # or any other value you deem appropriate

        cp = random.randint(0, X.shape[0] - 1 - sample_len) if sample_len < X.shape[0] else 0
        X = X[cp: cp + sample_len]
...
def collate_superv(data, max_len=None):
    ...
    print('xxxxxxxxxxxxxxxxxxxxxxx', X.shape, targets.shape, padding_masks.shape)
